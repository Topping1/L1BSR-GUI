...
#
# PATCHED: Includes clipping on output to prevent color artifacts from out-of-distribution pixels.
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("L1BSR Super-Resolution (PyTorch)")
        ...
        devs = ["cpu"]
        if torch.cuda.is_available(): devs.insert(0, "cuda")
        ...
        if os.path.isfile(default_weights): self.weights_edit.setText(default_weights)
    ...
    def choose_weights(self):
        ...
        if path: self.weights_edit.setText(path)
    ...
    def on_process(self):
        ...
        if not weights: QMessageBox.warning(self, "Weights", "Please select the model file."); return
